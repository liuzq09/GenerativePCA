            ...
            for ri in range(hparams.num_experiments):
                ...
                for key, x in xs_dict.items():
                    ...
                    if 0:
                        ...
                        V_batch = V_batch/y_batch_outer.shape[1]  # -np.repeat( np.eye(x_batch.shape[1], dtype=np.float32)[np.newaxis,:,:] , x_batch.shape[0], axis=0)    
                    
                        xcidx=np.diagonal(V_batch, axis1=1, axis2=2).argmax(1)       
                    for hparams.method in hparams.method_ls: 
                        x_main_batch = (np.sqrt(beta)*np.matmul((y_batch_outer)[:,:,np.newaxis],A_outer.transpose((0, 2, 1))).reshape((x_batch.shape[0],x_batch.shape[1])))
                        # x_main_batch =  1.0*x_batch 
                        # x_main_batch =  np.ones(x_batch.shape)
                        if 0:
                        ...
                        for k in range(maxiter):
                            ...
                            if 0:
                                ...
                                for si in range(y_batch_outer.shape[2]):
                                    for bi in range(y_batch_outer.shape[0]):   
                                        # print('x main shape',x_main_batch[bi,:].shape)
                                        Vx_batch[bi,:]=Vx_batch[bi,:]+y_batch_outer[bi,:,si]*(np.matmul(y_batch_outer[bi,:,si:si+1].transpose((1, 0)),x_main_batch[bi,:].reshape((x_main_batch.shape[1],1))).reshape((1)))
    ...
    #                            print('W shape',W.shape,'x shape:',x_est_batch.shape)
                ...
                if hparams.image_matrix > 0:
                    ...
                    if not os.path.exists(outputdir):
                        os.mkdir(outputdir)
                    hparams.savepath=outputdir+'spikedcov_beta_%0.2f_m_%d_r_%d.png'%(beta,hparams.num_outer_measurements,ri)
