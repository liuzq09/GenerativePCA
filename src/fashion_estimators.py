...
def wavelet_basis(path='./wavelet_basis_fashion.npy'):
    W_ = np.load(path)
...
def vae_estimator(hparams):
    ...
    z_batch, x_hat_batch, restore_path, restore_dict = fashion_model_def.vae_gen(hparams)
    ...
    def estimator(y_batch_val,z_batch_val,hparams):
        ...
        best_keeper = utils.BestKeeper_fashion(hparams)
...
def learned_estimator(hparams):
    ...
    sess = tf.Session()
    y_batch, x_hat_batch, restore_dict = fashion_model_def.end_to_end(hparams)
